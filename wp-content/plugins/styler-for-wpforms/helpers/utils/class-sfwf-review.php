...
class Sfwf_Review {
	...
	public static function ajax_handler() {
		...
		try {
			...
			update_user_meta( $user_id, '_sfwf_reviews_dismissed_triggers', $dismissed_triggers );
			update_user_meta( $user_id, '_sfwf_reviews_last_dismissed', current_time( 'mysql' ) );
	...
	public static function dismissed_triggers() {
		...
		$dismissed_triggers = get_user_meta( $user_id, '_sfwf_reviews_dismissed_triggers', true );
	...
	public static function already_did( $set = false ) {
		...
		return (bool) get_user_meta( $user_id, '_sfwf_reviews_already_did', true );
	}
				...
				'time_installed' => array(
					'triggers' => array(
						'one_week'     => array(
							...
							'link'       => 'https://wordpress.org/support/plugin/styler-for-wpforms/reviews/?filter=5#new-post',
							'pri'        => 10,
						),
						'one_month'    => array(
							'message'    => sprintf( $time_message, $current_user->user_login, __( '1 month', 'sfwf' ) ),
							'conditions' => array(
						...
						'three_months' => array(
							'message'    => sprintf( $time_message, $current_user->user_login, __( '3 months', 'sfwf' ) ),
							'conditions' => array(
	...
	public static function admin_notices() {
		...
		<script type="text/javascript">
			(function ($) {
				...
				$(document)
					.on('click', '.sfwf-notice .sfwf-dismiss', function (event) {
						var $this = $(this),
							reason = $this.data('reason'),
							notice = $this.parents('.sfwf-notice');
					...
					.ready(function () {
						setTimeout(function () {
							$('.sfwf-notice button.notice-dismiss').click(function (event) {
								dismiss('maybe_later');
							});
						}, 1000);
					});
	...
	public static function last_dismissed() {
		...
		return get_user_meta( $user_id, '_sfwf_reviews_last_dismissed', true );
	}
