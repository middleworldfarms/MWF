...
/**
 * Auto-repair subscription cache when customer views My Account subscriptions page
 * Fixes issue where _wcs_subscription_ids_cache user meta becomes empty/corrupted
 * causing subscriptions to disappear from customer view despite existing in database
 */
add_action('woocommerce_account_subscriptions_endpoint', 'mwf_auto_repair_subscription_cache');
function mwf_auto_repair_subscription_cache() {
    $user_id = get_current_user_id();
    ...
    // Get cached subscription IDs
    $cached_ids = get_user_meta($user_id, '_wcs_subscription_ids_cache', true);
    
    // If cache is empty string (not set) or empty array, rebuild it
    if ($cached_ids === '' || (is_array($cached_ids) && empty($cached_ids))) {
        
        // Get actual subscriptions from database
        global $wpdb;
        $actual_sub_ids = $wpdb->get_col($wpdb->prepare("
        ...
        // Only update if we actually found subscriptions
        if (!empty($actual_sub_ids)) {
            $actual_sub_ids = array_map('intval', $actual_sub_ids);
            update_user_meta($user_id, '_wcs_subscription_ids_cache', $actual_sub_ids);
            
            // Clear object cache to force fresh lookup
            wp_cache_delete('subscriptions_' . $user_id, 'subscriptions');
            
            // Log the repair for monitoring
            error_log(sprintf(
                '[MWF] Auto-repaired subscription cache for user %d. Found %d subscriptions: %s',
                $user_id,
                count($actual_sub_ids),
                implode(', ', $actual_sub_ids)
            ));
        }
    }
}
...
/**
 * Ensure subscription cache is updated when subscription is created or customer changes
 */
add_action('woocommerce_subscription_status_updated', 'mwf_update_subscription_cache_on_status_change', 10, 3);
function mwf_update_subscription_cache_on_status_change($subscription, $new_status, $old_status) {
    ...
    // Initialize if empty
    if ($cached_ids === '' || !is_array($cached_ids)) {
        $cached_ids = array();
    }
    
    $subscription_id = $subscription->get_id();
    
    // Add subscription to cache if not present
    if (!in_array($subscription_id, $cached_ids)) {
        $cached_ids[] = $subscription_id;
        rsort($cached_ids); // Sort descending like WCS does
        update_user_meta($user_id, '_wcs_subscription_ids_cache', $cached_ids);
