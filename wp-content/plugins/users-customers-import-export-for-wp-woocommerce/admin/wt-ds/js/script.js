var wbte_uimpexp_ds = {
	getAsset:function( args ) {
		let params = wbte_uimpexp_ds_js_params;
	...
	getIconSvg: async function( icon ) {
		return await fetch( wbte_uimpexp_ds_js_params.icon_base_url + icon + '.svg')
	...
	sanitizeNode:function( node ) {
		...
		Array.from(node.children).forEach(wbte_uimpexp_ds.sanitizeNode);
	}
}

var wbte_uimpexp_checkboxes = {
    Set:function(){
        jQuery(document).ready(function(){
            wbte_uimpexp_checkboxes.set_first_state();
            wbte_uimpexp_checkboxes.reg_click();
        });
    },
    set_first_state:function(){
        jQuery('.wbte_uimpexp_checkbox-master').each(function(){
            var group_id = jQuery(this).attr('data-checkbox-group-id');
            wbte_uimpexp_checkboxes.toggle_master_check( group_id );
        });
    },
    reg_click:function(){
        jQuery(document).on('click', '.wbte_uimpexp_checkbox input[type="checkbox"]', function() {
            
            let parent_elm = jQuery(this).parents('.wbte_uimpexp_checkbox');
            let group_id = parent_elm.attr('data-checkbox-group-id');

            if(parent_elm.hasClass('wbte_uimpexp_checkbox-master')){
                if(jQuery(this).is(':checked')){
                    jQuery('.wbte_uimpexp_checkbox[data-checkbox-group-id="'+group_id+'"] input[type="checkbox"]').prop('checked', true).attr('aria-checked', 'true');
                }else{
                    jQuery('.wbte_uimpexp_checkbox[data-checkbox-group-id="'+group_id+'"] input[type="checkbox"]').prop('checked', false).attr('aria-checked', 'false');
                }
            }
    ...
    toggle_master_check:function( group_id ) {
        let normal_checkboxes = jQuery('.wbte_uimpexp_checkbox-normal[data-checkbox-group-id="'+group_id+'"]');
        let master_checkbox = jQuery('.wbte_uimpexp_checkbox-master[data-checkbox-group-id="'+group_id+'"]');
...
var wbte_uimpexp_popup={
	Set:function() {	
		jQuery(function() {
			if( jQuery('.wbte_uimpexp_blanket').length ) {
				jQuery('.wbte_uimpexp_blanket').prependTo('body');
			} else {	
				jQuery('body').prepend('<div class="wbte_uimpexp_blanket"></div>');
			}

			wbte_uimpexp_popup.regPopupOpen();
			wbte_uimpexp_popup.regPopupClose();
		});
	},
	regPopupOpen:function() {
		jQuery(document).on('click', '[data-wbte_uimpexp_popup]', function(){
			var elm_id=jQuery(this).attr('data-wbte_uimpexp_popup');
			var elm=jQuery('[data-id="'+elm_id+'"]');
			if ( elm.length ) {

				/* Trigger a custom event after the popup trigger was clicked. */
				let popup_trigger_clicked = jQuery.Event('wbte_uimpexp_popup_trigger_clicked', {
			        detail: {
				...
				wbte_uimpexp_popup.showPopup(elm);
			}
		});
	},
	showPopup:function( popup_elm ) {
        ...
        if ( popup_elm.attr('data-overlay') === '1' ) {
		    jQuery('.wbte_uimpexp_blanket').show();
        }
	},
	hidePopup:function() {
		jQuery('.wbte_uimpexp_popup-close').trigger('click');
	},
	regPopupClose:function( popup_elm ) {
		jQuery(document).on('keyup', function( e ) {
			if ( 'Escape' === e.key ) {
				wbte_uimpexp_popup.hidePopup();
			}
		});
		jQuery(document).on('click', '.wbte_uimpexp_popup-close, .wbte_uimpexp_popup-cancel, .wbte_uimpexp_blanket', function(){
			jQuery('.wbte_uimpexp_blanket, .wbte_uimpexp_popup').hide();
		});
	}
}

wbte_uimpexp_popup.Set();

var wbte_uimpexp_product_tour = {
	elements:[],
	Set:function( group_id, step ) {
		jQuery(document).ready(function(){
			wbte_uimpexp_product_tour.initTour( group_id, step );
		});
	},
	initTour:function( group_id, step ) {
		...
		this.elements = jQuery('.wbte_uimpexp_product-tour[data-group-id="' + group_id + '"]');
		...
		this.elements.each(function(index){
			jQuery(this).find('.wbte_uimpexp_product-tour-step-count .total').text( totalElements );
			jQuery(this).find('.wbte_uimpexp_product-tour-step-count .current').text( jQuery(this).attr('data-step') );
		})

		jQuery('.wbte_uimpexp_product-tour').hide(); /* Hide all tours. */

		let elm = this.elements.eq( step - 1 );
		if ( elm.length ) {
			wbte_uimpexp_product_tour.showBox( elm );
		}
		this.regNext( group_id );
		this.regSkip( group_id );
	},
	regNext:function( group_id ) {
		jQuery(document).off('click', '.wbte_uimpexp_product-tour[data-group-id="' + group_id + '"] .wbte_uimpexp_product-tour-next-btn').on( 'click', '.wbte_uimpexp_product-tour[data-group-id="' + group_id + '"] .wbte_uimpexp_product-tour-next-btn', function() {
			let current_step = parseInt( jQuery(this).parents('.wbte_uimpexp_product-tour').attr('data-step') );
			let next_elm = wbte_uimpexp_product_tour.elements.eq( current_step );
			wbte_uimpexp_product_tour.elements.animate({'opacity':0}, 100, function(){
				jQuery(this).hide().css({'opacity':1});
			});

			setTimeout(function() {
				if ( next_elm.length ) {				
					wbte_uimpexp_product_tour.showBox( next_elm );
				}
			}, 200);
		});
	},
	regSkip:function( group_id ) {
		jQuery(document).off('click', '.wbte_uimpexp_product-tour[data-group-id="' + group_id + '"] .wbte_uimpexp_product-tour-skip-btn').on( 'click', '.wbte_uimpexp_product-tour[data-group-id="' + group_id + '"] .wbte_uimpexp_product-tour-skip-btn', function() {
			wbte_uimpexp_product_tour.elements.hide();

			let popupElm = jQuery(this).parents('.wbte_uimpexp_product-tour');
			let target_id = popupElm.attr('data-target-id');
			let targetElm = jQuery('#' + target_id);

			// Trigger a custom event.
			let product_tour_item_skipped_event = jQuery.Event('wbte_uimpexp_product_tour_item_skipped', {
		        detail: {
	...
	showBox:function( popupElm ) {
	    ...
	    // Trigger a custom event.
		let product_tour_item_visible_event = jQuery.Event('wbte_uimpexp_product_tour_item_visible', {
	        detail: {
	          target_element: targetElm,
	          popup_element: popupElm,
	        },
...
var wbte_uimpexp_segments = {
	Set:function(){
		jQuery(document).ready(function() {
			wbte_uimpexp_segments.register_click();
			wbte_uimpexp_segments.load_segment_state();
		});
	},
	register_click:function(){
		/* Create a custom event to trigger after segment was clicked. */
		let segment_clicked_event = jQuery.Event('wbte_uimpexp_segment_clicked', {
	        detail: {
	          element: null,
	          segment_target_id: '',
		...
		jQuery(document).on('click', '.wbte_uimpexp_segment', function(e){
			let elm = jQuery(this);

			/* Set segment state */
			let segment_parent = elm.parent('.wbte_uimpexp_segments');
			segment_parent.find('.wbte_uimpexp_segment').removeClass('active');
			elm.addClass('active');


			/* Display the content. */
			let segment_target_id = typeof segment_parent.attr('data-target-id') === 'undefined' ? '' : segment_parent.attr('data-target-id');
			if( '' === segment_target_id || ( segment_target_id && ! jQuery( '.wbte_uimpexp_segment_content_main[data-id="' + segment_target_id + '"]' ).length ) ) { /* Segment target element id not found, or element not exists. */
				return;
			}

			let segment_target_elm = jQuery( '.wbte_uimpexp_segment_content_main[data-id="' + segment_target_id + '"]' );
			segment_target_elm.find('.wbte_uimpexp_segment_content').hide(); /* Hide all containers first. */
			...
			segment_target_elm.find( '.wbte_uimpexp_segment_content[data-id="' + segment_item_target_id + '"]' ).fadeIn();
	...
	load_segment_state:function(){
		jQuery('.wbte_uimpexp_segment.active').each(function(e){
			jQuery(this).trigger('click');
		});
	}
}
wbte_uimpexp_segments.Set();

var wbte_uimpexp_submenu = {
	Set:function(){
		jQuery(document).ready(function() {
			wbte_uimpexp_submenu.register_click();
			wbte_uimpexp_submenu.load_menu_state();
		});
	},
	register_click:function() {		
		
		/* Create a custom event to trigger after tab was loaded. */
		let tab_loaded_event = jQuery.Event('wbte_uimpexp_tab_loaded', {
	        detail: {
	          element: null,
	          menu_target_id: '',
	          menu_item_target_id: '',
		...
		jQuery(document).on('click', '.wbte_uimpexp_submenu_item', function(e){
			...
			if( elm.hasClass('has_submenu') ) {
				...
				if(elm.hasClass('wbte_uimpexp_horizontal_submenu_main_item')) {
					return;
				}

				let submenu = elm.siblings('.wbte_uimpexp_subsubmenu');
				if( submenu.is(':visible') ) {
					submenu.hide();
					elm.find('.wbte_uimpexp_submenu_arrow').removeClass('open');
				}else{
					submenu.show();
					elm.find('.wbte_uimpexp_submenu_arrow').addClass('open');
				}

				return;
			}

			let menu_parent = elm.parents('.wbte_uimpexp_submenu_main');
			menu_parent.find('.wbte_uimpexp_submenu_item').removeClass('active');
			elm.addClass('active');

			/* For horizontal menu */
			if( elm.hasClass('wbte_uimpexp_horizontal_subsubmenu_item') ) {
				elm.parents('.wbte_uimpexp_horizontal_subsubmenu').siblings('.wbte_uimpexp_submenu_item').addClass('active');
			}


			/* Display the content. */
			let menu_target_id = typeof menu_parent.attr('data-target-id') === 'undefined' ? '' : menu_parent.attr('data-target-id');
			if( '' === menu_target_id || ( menu_target_id && ! jQuery( '.wbte_uimpexp_submenu_content_main[data-id="' + menu_target_id + '"]' ).length ) ) { /* Menu target element id not found, or element not exists. */
				return;
			}

			let menu_target_elm = jQuery( '.wbte_uimpexp_submenu_content_main[data-id="' + menu_target_id + '"]' );
			menu_target_elm.find('.wbte_uimpexp_submenu_content').hide(); /* Hide all containers first. */
			...
			menu_target_elm.find( '.wbte_uimpexp_submenu_content[data-id="' + menu_item_target_id + '"]' ).show();
	...
	load_menu_state:function(){
		jQuery('.wbte_uimpexp_submenu_item.active').each(function(e){
			jQuery(this).trigger('click');
		});
	}
}
wbte_uimpexp_submenu.Set();

var wbte_uimpexp_notify_msg = {
	...
	error:function( message, auto_close ) {
		...
		var er_elm=jQuery('<div class="wbte_uimpexp_notify_msg wbte_uimpexp_notify_msg_error">' + this.error_icon + message + '</div>');				
		this.set_notify(er_elm, auto_close);
	},
	success:function( message, auto_close ) {
		var auto_close = (auto_close!== undefined ? auto_close : true);
		var suss_elm = jQuery('<div class="wbte_uimpexp_notify_msg wbte_uimpexp_notify_msg_success">' + this.success_icon + message + '</div>');				
		this.set_notify(suss_elm, auto_close);
	},
	warning:function( message, auto_close ) {
		var auto_close = (auto_close!== undefined ? auto_close : true);
		var suss_elm = jQuery('<div class="wbte_uimpexp_notify_msg wbte_uimpexp_notify_msg_warning">' + this.warn_icon + message + '</div>');				
		this.set_notify(suss_elm, auto_close);
	},
	progress:function( message ) {
		var prog_elm = jQuery('<div class="wbte_uimpexp_notify_msg wbte_uimpexp_notify_msg_progress"><span class="spinner"></span> ' + message + '</div>');				
		this.set_notify(prog_elm, false, true);
		return prog_elm;
	},
	progress_complete:function( elm, message, auto_close ) {
		var auto_close = (auto_close!== undefined ? auto_close : true);
		elm.removeClass('wbte_uimpexp_notify_msg_progress').addClass('wbte_uimpexp_notify_msg_success');
		elm.html( this.success_icon + message );				
		this.set_notify(elm, auto_close);
	},
	progress_error:function( elm, message, auto_close ) {
		var auto_close = (auto_close!== undefined ? auto_close : true);
		elm.removeClass('wbte_uimpexp_notify_msg_progress').addClass('wbte_uimpexp_notify_msg_error');
	...
	set_notify:function( elm, auto_close, is_static ) {
		...
		elm.on('click',function(){
			wbte_uimpexp_notify_msg.fade_out(elm);
		});
