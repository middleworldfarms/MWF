...
class GPLVault_Log_Handler_File extends GPLVault_Log_Handler {
	...
	protected static function format_entry( $timestamp, $level, $message, $context ) {
		...
		if ( isset( $context['_legacy'] ) && true === $context['_legacy'] ) {
			...
			/**
			 * Filters logger message
			 *
			 * @see GPLVault_Log_Handler_File::format_entry()
			 *
	...
	public function clear( $handle ) {
		...
		do_action( 'gplvault_log_clear', $handle );
	...
	public function remove( $handle ) {
		...
		if ( isset( $logs[ $handle ] ) && $logs[ $handle ] ) {
			...
			if ( 0 === stripos( $file, realpath( trailingslashit( GV_UPDATER_LOG_DIR ) ) ) && is_file( $file ) && is_writable( $file ) ) { // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable, WordPress.WP.AlternativeFunctions.file_system_operations_is_writable
				$this->close( $file ); // Close first to be certain no processes keep it alive after it is unlinked.
				$removed = unlink( $file ); // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_unlink, WordPress.WP.AlternativeFunctions.unlink_unlink
			}
			do_action( 'gplvault_log_remove', $handle, $removed );
		}
		return $removed;
	}
	...
	protected function increment_log_infix( $handle, $number = null ) {
		...
		if ( is_writable( $rename_from ) ) { // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable, WordPress.WP.AlternativeFunctions.file_system_operations_is_writable
			return rename( $rename_from, $rename_to ); // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_rename, WordPress.WP.AlternativeFunctions.rename_rename
		}
	...
	public static function delete_logs_before_timestamp( $timestamp = 0 ) {
		...
		foreach ( $log_files as $log_file ) {
			...
			if ( $last_modified < $timestamp ) {
				@unlink( trailingslashit( GV_UPDATER_LOG_DIR ) . $log_file ); // @codingStandardsIgnoreLine.
			}
		}
	}
	...
	public static function get_log_files() {
		$files  = @scandir( GV_UPDATER_LOG_DIR ); // @codingStandardsIgnoreLine.
		$result = array();
