...
$product_id = ! empty( $product_object ) && is_callable(
	array(
		$product_object,
		'get_id',
	)
) ? $product_object->get_id() : $post->ID;
...
$sugg_price = empty( $last_category_rule_id ) ? ywcnp_get_suggest_price( $product_id ) : get_term_meta( $last_category_rule_id, '_ywcnp_suggest_price', true );
$min_price  = empty( $last_category_rule_id ) ? ywcnp_get_min_price( $product_id ) : get_term_meta( $last_category_rule_id, '_ywcnp_min_price', true );
$max_price  = empty( $last_category_rule_id ) ? ywcnp_get_max_price( $product_id ) : get_term_meta( $last_category_rule_id, '_ywcnp_max_price', true );
...
<?php
/**
 * DO_ACTION: ywcnp_after_admin_tab_single_product_metabox
 *
 * hook after single product metabox in admin.
 *
 */
do_action( 'ywcnp_after_admin_tab_single_product_metabox' );
?>
