...
if ( ! class_exists( 'YITH_Custom_Table' ) ) {
	...
	class YITH_Custom_Table extends WP_List_Table {
		 ...
		 * Constructor
		 *
		 * @param array/string $args array|string array or string of arguments.
		 *
		 * @since   1.0.0
		 * @see     WP_List_Table
		 */
		 ...
		 *
		 * @param array  $item array the row.
		 * @param string $column_name string the column name.
		 *
		 * @return  string
		 * @since   1.0.0
		 *
		...
		public function prepare_items() {
			...
			$total_items = $wpdb->get_var( //phpcs:ignore WordPress.DB.DirectDatabaseQuery
				"SELECT COUNT(*) FROM $count_table $count_where" //phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			);
			// Prepare query params, as usual current page, order by and order direction !
			$paged   = isset( $_GET['paged'] ) ? $select_limit * ( intval( $_GET['paged'] ) - 1 ) : 0; //phpcs:ignore WordPress.Security.NonceVerification
			$orderby = ( isset( $_GET['orderby'] ) && in_array( $_GET['orderby'], array_keys( $this->get_sortable_columns() ), true ) ) ? sanitize_text_field( wp_unslash( $_GET['orderby'] ) ) : $this->options['select_order']; //phpcs:ignore WordPress.Security.NonceVerification

			$order_dir = ( '' !== $this->options['select_order_dir'] ) ? $this->options['select_order_dir'] : 'asc';
			$order     = ( isset( $_GET['order'] ) && in_array( $_GET['order'], array( 'asc', 'desc' ), true ) ) ? sanitize_text_field( wp_unslash( $_GET['order'] ) ) : $order_dir; //phpcs:ignore WordPress.Security.NonceVerification

			$this->items = $wpdb->get_results( //phpcs:ignore WordPress.DB.DirectDatabaseQuery
				$wpdb->prepare(
					"
                        SELECT $select_columns FROM $select_table $select_where $select_group ORDER BY $orderby $order LIMIT %d OFFSET %d", //phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
					$select_limit,
					$paged
				),
				ARRAY_A
		 ...
		 *
		 * @param mixed $item array the row.
		 *
		 * @return  void
		 * @since   1.0.0
		 *
		...
		protected function single_row_columns( $item ) {
			...
			foreach ( $columns as $column_name => $column_display_name ) {
				...
				if ( 'cb' === $column_name ) {
					...
					echo "<td $attributes >";
					echo call_user_func_array( $this->options['custom_columns'][ 'column_' . $column_name ], array( $item, $this ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					echo '</td>';
				} else {
					echo "<td $attributes>";
